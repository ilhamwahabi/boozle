...
function RoomDetail(props) {
  ...
  const [chats, setChats] = useState(data.chat.map(chat => renderChat(chat)));
  ...
  const renderOpponentButton = () => {
    return(
      <button className="h-16 w-40 bg-gray-700 m-2 text-white rounded-large" disabled={true}>{data.oppApproval ? "Opponent Ready" : "Wait For Opponent Accept"}</button>
    );
  };
    ...
    if (bothAccepted) {
      return (
        <div className="flex justify-center space-between">
          <button className="h-16 w-full bg-gray-700 my-2 text-white rounded-large" disabled={true}>{"Two days to go..."}</button>
        </div>
      )
    } else {
      return(
  ...
  return (
    <div className="flex flex-col items-center bg-main min-h-full flex-1">
      ...
      {/* <div className="text-center text-lg mb-2 font-medium self-start"></div> */}
          ...
          <div className="w-full flex flex-1 flex-col justify-between items-start shadow-lg bg-white rounded-large">
            ...
            <div className="w-full border border-l-0 border-r-0 border-b-0">
              <div className="p-2 w-full">
                <input placeholder="Type to chat..." className="self-start pl-2 border border-8 rounded-large h-8 w-full" onChange={handleChange} onKeyDown={handleKeyDown} value={message}></input>
              </div>
            </div>
          </div>
        </div>
